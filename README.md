# 다시 풀기 / 지하철 노선도 경로 조회 미션 

- 우아한테크코스 3기 지원 과정의 최종 테스트 미션을 시간에 제약 없이 리펙토링 해보고자 다시 풀이한 저장소입니다.

- 이번에는 구현보다도 천천히 제 코드의 근거를 찾는 연습을 해보고 싶습니다. 왜 이렇게 짰고, 반대로 왜 이렇게 하지 않았는지를 천천히 생각해보려 합니다.

## 다시 풀기로 수정한 부분

1. 불필요한 인스턴스를 제거하자 -> 싱글톤으로 변경

2. Controller의 집중된 기능 로직 분리 -> 최단 거리를 계산하는 로직을 SearchController에서 들고 있을 필요가 있었을까
   최단 거리를 계산하여 최단 거리를 반환하는 로직을 ShortestPath 클래스를 정의하여 분리
   
3. 예외 처리를 상위 함수로 -> 하위 함수에서 try - catch 하는 로직을 호출 함수에서 처리할 수 있었고, 중복도 피하고 가독성도 더 좋아졌다.
    SearchController의 getDeparture / getArrival 메소드 참고
